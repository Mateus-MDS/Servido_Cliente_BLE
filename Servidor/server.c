...
#define ssd_I2C_SDA_PIN 15         // SDA do ssd  
#define ssd_I2C_SCL_PIN 14         // SCL do ssd  
#define I2C_ssd_PORT i2c1 
ssd1306_t ssd;
...
int main()
{
    ...
    stdio_init_all(); 
    i2c_init(I2C_ssd_PORT, 400000);
    gpio_set_function(ssd_I2C_SDA_PIN, GPIO_FUNC_I2C);
    gpio_set_function(ssd_I2C_SCL_PIN, GPIO_FUNC_I2C);
    gpio_pull_up(ssd_I2C_SDA_PIN);
    gpio_pull_up(ssd_I2C_SCL_PIN);
    ssd1306_init(&ssd, 128, 64, false, 0x3C, I2C_ssd_PORT);
    ssd1306_config(&ssd);
    ...
    */
        ...
        while (true) { // Loop infinito alternativo
            ...
            char Temp_string[20]; 
            snprintf(Temp_string, sizeof(Temp_string), "Temp: %.2f C", (float)current_temp/100);
            ssd1306_rect(&ssd, 3, 3, 122, 60, color, !color);
            ssd1306_line(&ssd, 3, 25, 123, 25, color);
            ssd1306_line(&ssd, 3, 37, 123, 37, color);
            ssd1306_draw_string(&ssd, "SERVIDOR", 33, 6);
